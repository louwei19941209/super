...
<form method="post" action="dept/update.do" onsubmit="return validateSubmitForm(this)">
    ...
    <div style="margin-left: 10px;margin-top: 10px;margin-bottom: 5px;">
        <shiro:hasPermission name="dept:add">
            <a class="btn btn-primary" href="dept/addPage.do?rel=${param.rel}" rel="${param.rel }_box" target="ajax"><i
                    class="boot_icon-plus-sign boot_icon-white"></i> 添加顶级部门</a>
            ...
            <a class="btn btn-primary" href="${url_addxjdept }" rel="${param.rel }_box" target="ajax"><i
                    class="boot_icon-plus-sign boot_icon-white"></i> 添加下级部门</a>
        </shiro:hasPermission>
        <shiro:hasPermission name="dept:delete">
            ...
            <a class="btn btn-danger" target="ajaxTodo" href="${url_delxjdept }"
               title="您确定删除部门<font color='red'>[${requestScope.dept.deptName }]</font>吗？如果此部门有下级部门，或者此部门有用户，将无法删除！"><i
                    class="boot_icon-remove-sign boot_icon-white"></i> 删除</a>
        </shiro:hasPermission>
    ...
    <table class="table table-nobordered ">
        ...
        <tr>
            ...
            <td>
                <input type="text" name="deptSort" maxlength="3" class="required digits" min="1" max="999" size="25"
                       value="${requestScope.dept.deptSort}"/>
        ...
        <tr>
            ...
            <td>
                <input type="text" name="deptName" class="required" maxlength="50" size="25"
                       value="<c:out value="${requestScope.dept.deptName}"/>"/>
            </td>
        </tr>
        <tr>
            <th>电话：</th>
            <td>
                <input type="text" name="deptPhone" maxlength="20" size="25"
                       value="<c:out value="${requestScope.dept.deptPhone}"/>"/>
        ...
        <tr>
            ...
            <td>
                <input type="text" name="deptFax" maxlength="20" size="25"
                       value="<c:out value="${requestScope.dept.deptFax}"/>"/>
        ...
        <tr>
            <th>地址：</th>
            <td>
                <textarea name="deptAddress" cols="80" rows="3" maxlength="100"><c:out
                        value="${requestScope.dept.deptAddress}"/></textarea>
            </td>
        </tr>
            ...
            <td>
                <a href="user/lookUpPage.do?type=1" lookupGroup="user" title="用户查询">
                    <input type="text" readonly="readonly" toName="user.trueName"
                           value="<my:userName id="${requestScope.dept.leadUid}"/>"/>
                </a>
                <a class="easyui-linkbutton clearLookup" icon="icon-cancel" plain="true" href="javascript:;"
                   clearLookup="user" title="清空"></a>
        ...
        <tr>
            ...
            <td>
                <a href="dept/lookUpPage.do?type=3" lookupGroup="dept" title="上级部门查询">
                    ...
                    <input class="required" type="text" readonly="readonly" size="25" toName="dept.deptName"
                           value="无上级"/></a><a class="btnLook" href="dept/lookUpPage.do" lookupGroup="dept">查找带回</a>
                <input type="hidden" name="superId" size="25" toName="dept.id" value="0"/>

                </c:when>
                <c:otherwise>
                    <input type="text" readonly="readonly" size="25" toName="dept.deptName"
                           value="<my:outDeptName id="${requestScope.dept.superId}"/>"/>
                    <input type="hidden" name="superId" size="25" toName="dept.id"
                           value="${requestScope.dept.superId}"/>
                </c:otherwise>
                </c:choose>
                </a>
                <a class="easyui-linkbutton clearLookup" icon="icon-cancel" plain="true" href="javascript:;"
                   clearLookup="dept" title="清空"></a>
        ...
        <tr>
            <th>备注：</th>
            <td>
                <textarea name="deptDesc" cols="80" rows="2" maxlength="200"><c:out
                        value="${requestScope.dept.deptDesc}"/></textarea>
            </td>
        </tr>
