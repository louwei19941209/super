...
<form action="dept/add.do" method="post" onsubmit="return validateSubmitForm(this)">
    ...
    <div style="margin-left: 20px;margin-top: 10px">
        ...
        <h4><p class="text-info">添加顶级部门</p></h2>
            ...
            <c:otherwise>
            <h4><p class="text-info">添加部门：上级部门为：<span style="color: red">[ <my:outDeptName
                    id="${param.superId }"/> ]</span></p></h4>
            </c:otherwise>
            </c:choose>
    </div>
    ...
    <table class="table table-nobordered ">
        ...
        <tr>
            ...
            <td>
                <input type="text" name="deptSort" class="easyui-numberbox" required="true" min="1" max="999"
                       maxlength="3"/>
        ...
        <tr>
            <th>部门名称：</th>
            <td>
                <input type="text" name="deptName" class="easyui-validatebox" required="true"
        ...
        <tr>
            <th>所属单位：</th>
            <td>
                <a href="sy_company/lookUpPage.do?type=3" lookupGroup="company" title="单位">

                    <input type="text" readonly="readonly"
                           toName="company.companyName" class="easyui-validatebox" required="true"/>

                </a>
                <input type="hidden" name="companyId" toName="company.id"/>
                <a class="easyui-linkbutton clearLookup" icon="icon-cancel" plain="true" href="javascript:;"
                   clearLookup="company" title="清空"></a>
            </td>
        </tr>
        <tr>
            <th>上级部门：</th>
                ...
                <a href="dept/lookUpPage.do?type=3" lookupGroup="dept" title="上级部门查询">
                    ...
                    <c:choose>
                        <c:when test="${empty param.superId }">
                            <input type="text" value="顶级部门" readonly="readonly" toName="dept.deptName"
                                   class="easyui-validatebox" required="true"/>
                        </c:when>
                        <c:otherwise>
                            <input type="text" value="<my:outDeptName id="${param.superId }" />" readonly="readonly"
                                   toName="dept.deptName" class="easyui-validatebox" required="true"/>
                        </c:otherwise>
                    </c:choose>

                </a>
                <input type="hidden" name="superId" toName="dept.id"
                       value="<c:out value="${param.superId }" default="0"/>"/>
                <a class="easyui-linkbutton clearLookup" icon="icon-cancel" plain="true" href="javascript:;"
