/**
 * 包容分支，判断分支和并行的结合，效果：有条件判断，但是不会只执行一条，只要条件符合就可以。
 */
draw2d.InclusiveGateway=function(){
	var _url="resource/js/workflowDesigner/css/icons/gateway_inclusive.png";
...
draw2d.InclusiveGateway.prototype=new draw2d.Node();
draw2d.InclusiveGateway.prototype.type="draw2d.InclusiveGateway";
draw2d.InclusiveGateway.prototype.generateId=function(){
	this.id="inclusiveGateway"+Sequence.create();
	this.gatewayId=this.id;
	this.gatewayName='inclusive Gateway';
};
draw2d.InclusiveGateway.prototype.createHTMLElement=function(){
	var item = draw2d.ResizeImage.prototype.createHTMLElement.call(this);
	return item;
};
draw2d.InclusiveGateway.prototype.setDimension=function(w, h){
	draw2d.ResizeImage.prototype.setDimension.call(this, w, h);
};
draw2d.InclusiveGateway.prototype.setWorkflow=function(_4fe5){
...
draw2d.InclusiveGateway.prototype.figureDoubleClick=function(){
	var data = {event:this};
	var event = data.event;
	nodeid = event.getId();
	openProperties(nodeid,'gateway/inclusiveGatewayProperties');
};
draw2d.InclusiveGateway.prototype.getContextMenu=function(){
...
draw2d.InclusiveGateway.prototype.toXML=function(){
	
	var newName = trim(this.gatewayName);
	
	var xml='<inclusiveGateway id="'+this.gatewayId+'" ';
...
draw2d.InclusiveGateway.prototype.toBpmnDI=function(){
	var w=this.getWidth();
