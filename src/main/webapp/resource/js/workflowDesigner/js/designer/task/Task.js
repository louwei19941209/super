draw2d.Task = function() {
	...
	this.rules=null;
	this.isclude="";
	this.listeners=new draw2d.ArrayList();//执行监听器
	
	this.forms=new draw2d.ArrayList();
	this.candidateUsers=new draw2d.ArrayList();
	this.candidateGroups=new draw2d.ArrayList();
	
	//以下为多实例属性,流程会默认创建三个流程父任务变量：
	//nrOfInstances:实例总数，
	//nrOfActiveInstances:当前活跃的，也就是当前还未完成的，对于顺序的多实例，此值总是1
	//nrOfCompleteInstances:已完成的实例个数，
	this.isSequential=null;//true:顺序执行，false:并行
	this.loopCardinality=null;//直接指定实例数量，可以数字，或表达式
	this.collection=null;//一个类型为List集合的流程变量名称，根据此集合的size创建多少个实例,和loopCardinality只能用一种方式
	this.elementVariable=null;//遍历集合元素是每个元素的临时变量名称，
							  //例如：collection=[user1,user2,user3], elementVariable=userId 那么在用户任务activiti:assignee="${userId}"分配任务执行人
	this.completionCondition=null;//多实例活动结束的条件，默认为完成全部实例，当表达式返回true时结束多实例活动，
								 //例：${nrOfCompleteInstances/nrOfInstances>=0.6} 说明当有60%的任务完成时，会完成此多实例，删除其他未完成的，继续下面的流程
...
draw2d.Task.prototype.getContextMenu=function(){
	...
	menu.appendMenuItem(new draw2d.ContextMenuItem("属性", "properties-icon",data,function(x,y)
	{
		...
		if(typeof openProperties != "undefined"){
			openProperties(nodeid,'taskProperties','task');
		}
	}));
...
draw2d.Task.prototype.getForm=function(id){
...
draw2d.Task.prototype.deleteForm=function(id){
...
draw2d.Task.prototype.addForm=function(form){
	this.forms.add(form);
};
...
draw2d.Task.prototype.setForms=function(forms){
	this.forms = forms;
};
