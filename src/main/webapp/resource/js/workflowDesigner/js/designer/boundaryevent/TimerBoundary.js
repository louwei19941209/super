...
draw2d.TimerBoundary=function(_url){
	...
	this.name="";
	this.cancelActivity=null;//是否中断活动
	this.attached=null;//attachedToRef 依附到的活动id
...
draw2d.TimerBoundary.prototype.generateId=function(){
	this.id="boundarytimer"+Sequence.create();
...
draw2d.TimerBoundary.prototype.figureDoubleClick=function(){
	...
	var tid = event.getId();
	openProperties(tid,'boundaryevent/timerBoundaryEventProperties');
};
...
draw2d.TimerBoundary.prototype.toXML=function(){
	...
	if(attached != null && attached != "")
		attached = attached;
	var xml='<boundaryEvent id="'+this.boundaryId+'" name="'+this.name+'"  cancelActivity="'+this.cancelActivity+'" attachedToRef="'+attached+'">\n';
