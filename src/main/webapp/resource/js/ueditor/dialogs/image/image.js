...
//在线管理 分页
var page=1;// 当前页数
var isLastPage=false;//最后一页
var imgName="";//图片名称
var imgType="";//图片类型（保存目录）
(function () {
    var g = $G,
...
//********拓展在线管理， 添加搜索，分页****/
    var bt_ss = g("imgManager_bt_ss");//搜索按钮
    bt_ss.onclick = function () {
    	
    	imgType= g( "img_saveType" ).value;
		imgName= g( "imgName" ).value;
    	page=1;
    	isLastPage=false;
    	selectImgParam(0);//点击搜索
    };
    var select_pre = g("select_pre");//上一页
    select_pre.onclick = function () {
    	if(page<2){
    		alert("已经在第一页了!");
    		return false;
    	}
    ...
    var select_next = g("select_next");//下一页
    select_next.onclick = function () {
    	if(isLastPage){
    		alert("已经是最后一页了!");
    		return false;
    	}
...
//********自定义拓展结束**********// 
    ...
    function insertSearch(id, catchRemote) {
        ...
        for (var i = 0, ci; ci = imgs[i++];) {
            if (ci.getAttribute("selected")) {
                ...
                url=url.replace(editor.options.webRootPath,""); //将本地图片去掉域名
                img.src = url;
                img._src = url;
                imgObjs.push(img);
            }
    ...
    function insertSingle() {
        ...
        url.value=url.value.replace(editor.options.webRootPath,""); //将本地图片去掉域名
    ...
    function insertBatch() {
        ...
        for (var i = 0, ci; ci = imageUrls[i++];) {
           ...
           // tmpObj._src = tmpObj.src = editor.options.imagePath + ci.url;
            tmpObj._src = tmpObj.src = ci.url;//不需要使用绝对地址
            alert(tmpObj._src);
            imgObjs.push(tmpObj);
        }
        insertImage(imgObjs);
